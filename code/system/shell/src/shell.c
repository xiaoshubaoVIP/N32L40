...
extern const unsigned int shellVar$$Base;
extern const unsigned int shellVar$$Limit;
#elif defined(__ICCARM__) || defined(__ICCRX__)
#pragma section = "shellCommand"
#pragma section = "shellVar"
#elif defined(__GNUC__)
extern const unsigned int _shell_command_start;
extern const unsigned int _shell_command_end;
extern const unsigned int _shell_var_start;
extern const unsigned int _shell_var_end;
#endif
#else
extern const ShellCommand shellCommandList[];
extern const unsigned short shellCommandCount;
extern const ShellCommand shellVarList[];
extern const unsigned short shellVarCount;
#endif
...
static const char *shellText[] =
    {
        ...
        [SHELL_TEXT_CMD_NOT_FOUND] =
            "Command not Found. Try 'help' for more.\r\n",
...
void shellInit(Shell *shell, char *buffer, unsigned short size)
{
    ...
    shell->VarList.base = (ShellCommand *)(&shellVar$$Base);
    shell->VarList.count = ((unsigned int)(&shellVar$$Limit) - (unsigned int)(&shellVar$$Base)) / sizeof(ShellCommand);
    ...
    shell->VarList.base = (ShellCommand *)(__section_begin("shellVar"));
    shell->VarList.count = ((unsigned int)(__section_end("shellVar")) - (unsigned int)(__section_begin("shellVar"))) / sizeof(ShellCommand);
#elif defined(__GNUC__)
    ...
    shell->VarList.base = (ShellCommand *)(&_shell_var_start);
    shell->VarList.count = ((unsigned int)(&_shell_var_end) - (unsigned int)(&_shell_var_start)) / sizeof(ShellCommand);
#else
...
#else
#define shellWritePrompt(shell, newline)
#endif
...
{
    ShellCommand *base = (ShellCommand *)shell->VarList.base;
    shellWriteString(shell, shellText[SHELL_TEXT_VAR_LIST]);
    for (short i = 0; i < shell->VarList.count; i++)
    {
 ...
 * @return int 返回变量值
 */
int shellSetVar(int argc, char *argv[])
{
    ...
    if (argc < 3)
    {
        shellWriteString(shell, "usage: set <var> <value>\r\n");
        shellListVar(shell);
        return 0;
    }
    else
    {
        char *name = (char *)shellExtParsePara(shell, argv[1]);
        int value;
        ...
        value = shellExtParsePara(shell, argv[2]);
        return shellSetVarValue(shell, command, value);
    }
}
SHELL_EXPORT_CMD(
    SHELL_CMD_PERMISSION(0) | SHELL_CMD_TYPE(SHELL_TYPE_CMD_MAIN) | SHELL_CMD_DISABLE_RETURN,
    set, shellSetVar, "set <var> <value>");
 ...
 * @return int 返回变量值
 */
int shellGetVar(int argc, char *argv[])
{
    Shell *shell = shellGetCurrent();
    if (shell == NULL)
    {
    ...
    {
        shellWriteString(shell, "usage: get <var>\r\n");
        shellListVar(shell);
        return 0;
    }
}
...
unsigned int shellRunCommand(Shell *shell, ShellCommand *command)
{
    ...
    return command->data.cmd.function(shell->parser.paramCount, shell->parser.param);
}
...
void shellExec(Shell *shell)
{
    ...
    ShellCommand *command = shellSeekCommand(shell,
                                             ...
                                             &shell->commandList,
                                             0);
    if (command != NULL && command->attr.attrs.type == SHELL_TYPE_CMD_MAIN)
    {
        shellRemoveParamQuotes(shell);
        shell->info.retVal = command->data.cmd.function(shell->parser.paramCount, shell->parser.param);
    }
    else
    {
...
#if SHELL_ENTER_LF == 1
SHELL_EXPORT_KEY(SHELL_CMD_PERMISSION(0) | SHELL_CMD_ENABLE_UNCHECKED,
                 0x0A000000, shellExec, enter);
#endif
#if SHELL_ENTER_CR == 1
SHELL_EXPORT_KEY(SHELL_CMD_PERMISSION(0) | SHELL_CMD_ENABLE_UNCHECKED,
                 0x0D000000, shellExec, enter);
#endif
